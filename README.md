...
│       ├── lnsrch_solver.hpp
│       └── detail/
└── bicycle/                       # boyle::bicycle — Motion Planning
    └── planners/
        ├── route_line.hpp
        ├── path.hpp
        ├── motion.hpp
        ...
        ├── border.hpp
        ├── dualism.hpp
        ├── route_line_cubic_acc_planner.hpp
        ├── route_line_quintic_acc_planner.hpp
        ├── route_line_cubic_offset_planner.hpp
        └── route_line_quintic_offset_planner.hpp
```
...
| ----------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `vec2.hpp`        | `Boyle::math_vec2`        | Fixed-size 2D vector `Vec2<T>` with `euclidean()`, `radian()`, `rotate()`, `crossProj()`, `normalized()`, optional AVX-512 SIMD |
...
| `qdldl_dcmp.hpp`          | `Boyle::math_qdldl_dcmp`          | Sparse LDL^T decomposition `QdldlDcmp<T>` (via qdldl) for solving symmetric quasi-definite systems stored in upper-triangular CSC format |
...
| `fixed.hpp`                 | `Boyle::math_fixed`                 | Fixed-point number `Fixed<ValueType, IntermediateType, FractionBits>` with full arithmetic, math functions (`sqrt`, `sin`, `cos`, `exp`, `log`, …), `std::numeric_limits` and `std::formatter` support. Predefined aliases: `Fixed32f8`, `Fixed32f16`, `Fixed32f24` |
...
| File                | Target                        | Description                                                                           |
| ------------------- | ----------------------------- | ------------------------------------------------------------------------------------- |
| `osqp_problem.hpp`  | `Boyle::cvxopm_osqp_problem`  | Sparse QP problem `OsqpProblem<T, I>`: minimize ½xᵀPx + qᵀx subject to l ≤ Ax ≤ u     |
...
### `boyle::bicycle` — Motion Planning

| File             | Target                       | Description                                                                               |
| ---------------- | ---------------------------- | ----------------------------------------------------------------------------------------- |
| `route_line.hpp` | `Boyle::bicycle_route_line` | Reference line `RouteLine<T>` as a piecewise quintic curve for autonomous driving         |
| `path.hpp`       | `Boyle::bicycle_path`       | 2D path `Path<T>` with `tangent()`, `normal()`, `curvature()`, `inverse()`                |
| `motion.hpp`     | `Boyle::bicycle_motion`     | Time–arc-length mapping `Motion<T>` with `velocity()`, `accel()`, `jerk()`                |
| `fence.hpp`      | `Boyle::bicycle_fence`      | Time–space constraints `HardFence<T>` / `SoftFence<T>` for longitudinal planning          |
| `border.hpp`     | `Boyle::bicycle_border`     | Spatial constraints `HardBorder<T>` / `SoftBorder<T>` with chirality for lateral planning |
| `dualism.hpp`    | `Boyle::bicycle_dualism`    | Enumerations: `Chirality` (LEFT/RIGHT), `Actio` (BLOCKING/YIELDING)                       |

#### Motion Planners (`bicycle/planners/`)

| File                                     | Target                                              | Description                                                                     |
| ----------------------------------------- | ---------------------------------------------------- | --------------------------------------------------------------------------------- |
| `route_line_cubic_acc_planner.hpp`      | `Boyle::bicycle_route_line_cubic_acc_planner`      | Cubic acceleration planner — generates longitudinal motion via QP optimization  |
| `route_line_quintic_acc_planner.hpp`    | `Boyle::bicycle_route_line_quintic_acc_planner`    | Quintic acceleration planner — smoother longitudinal motion via QP optimization |
| `route_line_cubic_offset_planner.hpp`   | `Boyle::bicycle_route_line_cubic_offset_planner`   | Cubic offset planner — generates lateral path offset via QP optimization        |
| `route_line_quintic_offset_planner.hpp` | `Boyle::bicycle_route_line_quintic_offset_planner` | Quintic offset planner — smoother lateral path offset via QP optimization       |
...
OsqpProblem<double, int> osqp_problem(2, 3);   // 2 variables, 3 constraints
osqp_problem.updateQuadCostTerm(0, 0, 2.0);
osqp_problem.updateQuadCostTerm(1, 1, 1.0);
osqp_problem.updateQuadCostTerm(0, 1, 1.0);
osqp_problem.updateLinCostTerm(0, 1.0);
osqp_problem.updateLinCostTerm(1, 1.0);
...
auto [result, info] = solver.solve(osqp_problem);
...
auto [result2, info2] = solver.solve(osqp_problem, prim_vars_0, dual_vars_0);
```
...
| `boyle::bicycle`     | Motion planning: paths, route lines, motions, borders, fences, planning models |
...
| ------------------------------------------------------------- | ---------- | ----------------------------------------------- | ------------------------------------------------ |
| [Boost](https://www.boost.org/)                               | 1.91.0     | Unordered containers, hash                      | https://www.boost.org/                           |
...
| [zpp_bits](https://github.com/eyalz800/zpp_bits)              | 4.7.6      | Header-only binary serialization library        | https://github.com/eyalz800/zpp_bits             |
