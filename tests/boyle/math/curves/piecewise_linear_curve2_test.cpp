...
TEST_CASE("LeftSemiCircleTest") {
    ...
    for (std::size_t i{1}; i < kNumAnchors; ++i) {
        ...
        CHECK_EQ(semi_circle.curvature(s_exact), doctest::Approx(0.5).epsilon(1E-4));
    }
...
TEST_CASE("PeriodicSeamNormalTest") {
    // A sharply asymmetric closed shape: the turn direction from the wrapped previous segment
    // (P4->P0) into the first segment (P0->P1) is a left turn, while the turn from the first
    // segment into the second (P1->P2) is a right turn. This makes the seam-vertex normal and
    // curvature sign-sensitive to whether the wrapped neighbor or the naive forward neighbor is
    // used, so a stale one-sided formula would be caught here instead of accidentally agreeing.
    const std::vector<Vec2d> anchor_points{Vec2d{0.0, 0.0},  Vec2d{1.0, 1.0},  Vec2d{2.0, 0.0},
                                           Vec2d{0.0, -2.0}, Vec2d{-2.0, 0.0}, Vec2d{0.0, 0.0}};

    const PiecewiseLinearCurve<Vec2d> curve{periodic_tag{}, anchor_points};
    const PiecewiseLinearCurve<Vec2d> nonperiodic_curve{anchor_points};

    CHECK(curve.periodic());
    CHECK_FALSE(nonperiodic_curve.periodic());

    const Vec2d expected_normal{-1.0 / std::sqrt(2.0), 1.0 / std::sqrt(2.0)};
    CHECK(curve.normal(curve.minS()).identicalTo(expected_normal, 1E-6));

    constexpr double kExpectedCurvature{0.4472135955};
    CHECK_EQ(curve.curvature(curve.minS()), doctest::Approx(kExpectedCurvature).epsilon(1E-6));

    constexpr double kOffset{0.1};
    const Vec2d expected_eval{curve.eval(curve.minS()) + expected_normal * kOffset};
    CHECK(curve.eval(curve.minS(), kOffset).identicalTo(expected_eval, 1E-6));
}

TEST_CASE("PeriodicWraparoundTest") {
    constexpr double kRadius{3.0};
    constexpr std::size_t kNumAnchors{41};
    constexpr double kStep{2.0 * M_PI / (kNumAnchors - 1)};

    std::vector<Vec2d> anchor_points(kNumAnchors);
    std::ranges::generate(anchor_points, [t = -kStep, h = kStep]() mutable noexcept -> Vec2d {
        t += h;
        return Vec2d{kRadius * std::cos(t), kRadius * std::sin(t)};
    });

    const PiecewiseLinearCurve<Vec2d> curve{periodic_tag{}, anchor_points};

    CHECK(curve.periodic());
    CHECK_EQ(curve.minS(), doctest::Approx(0.0).epsilon(kEpsilon));

    const double perimeter{curve.maxS() - curve.minS()};
    for (double dt : {-3.0 * perimeter, -perimeter, perimeter, 2.0 * perimeter}) {
        for (double s : linspace(curve.minS() + 1E-3, curve.maxS() - 1E-3, 9)) {
            CHECK(curve.eval(s + dt).identicalTo(curve.eval(s), 1E-6));
            CHECK(curve.tangent(s + dt).identicalTo(curve.tangent(s), 1E-4));
            CHECK(curve.normal(s + dt).identicalTo(curve.normal(s), 1E-4));
        }
    }
}
