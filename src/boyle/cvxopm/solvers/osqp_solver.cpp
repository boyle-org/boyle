...
auto OsqpSolver<OSQPFloat, OSQPInt>::solve(
    ...
    const auto num_vars{static_cast<OSQPInt>(osqp_problem.num_variables())};
    const auto num_cons{static_cast<OSQPInt>(osqp_problem.num_constraints())};
    const ::boyle::math::pmr::CscMatrix<OSQPFloat, OSQPInt> objective_matrix{
        osqp_problem.m_objective_matrix
    };
    const ::boyle::math::pmr::CscMatrix<OSQPFloat, OSQPInt> constraint_matrix{
    ...
    const OSQPCscMatrix A{
        .m = num_cons,
        .n = num_vars,
        .p = const_cast<OSQPInt*>(constraint_matrix.outerIndices().data()),
        .i = const_cast<OSQPInt*>(constraint_matrix.innerIndices().data()),
        .x = const_cast<OSQPFloat*>(constraint_matrix.values().data()),
        .nzmax = static_cast<OSQPInt>(constraint_matrix.nnzs()),
        .nz = -1,
        .owned = 0
    };
