...
struct LnsrchSolver final {
    ...
    LnsrchSolver(const LnsrchSolver& other) noexcept = delete;
    auto operator=(const LnsrchSolver& other) noexcept -> LnsrchSolver& = delete;
    LnsrchSolver(LnsrchSolver&& other) noexcept = delete;
    auto operator=(LnsrchSolver&& other) noexcept -> LnsrchSolver& = delete;
    ~LnsrchSolver() noexcept = default;

    [[using gnu: always_inline]]
    explicit LnsrchSolver(const Settings<value_type, index_type>& c_settings) noexcept
        : settings{c_settings} {}
