/**
 * @file route_line_quintic_acc_planner.hpp
...
#include "boyle/bicycle/planners/fence.hpp"
#include "boyle/bicycle/planners/motion.hpp"
#include "boyle/common/logging.hpp"
...
class RouteLineQuinticAccPlanner final {
  public:
    using value_type = T;
    using scalar_type = T;
    ...
    RouteLineQuinticAccPlanner(const RouteLineQuinticAccPlanner& other) noexcept = delete;
    auto operator=(const RouteLineQuinticAccPlanner& other) noexcept
        -> RouteLineQuinticAccPlanner& = delete;
    RouteLineQuinticAccPlanner(RouteLineQuinticAccPlanner&& other) noexcept = delete;
    auto operator=(RouteLineQuinticAccPlanner&& other) noexcept
        -> RouteLineQuinticAccPlanner& = delete;
    ~RouteLineQuinticAccPlanner() noexcept = default;
    ...
    explicit RouteLineQuinticAccPlanner(
        ...
        : m_num_samples(sample_ts.size()), m_time_scale{sample_ts.back() - sample_ts.front()},
          m_osqp_problem(sample_ts.size() * 3, sample_ts.size() * 5, settings.memory_resource),
          m_settings{settings},
    ...
    [[using gnu: pure, always_inline]]
    auto osqp_problem() const noexcept
        -> const ::boyle::cvxopm::OsqpProblem<scalar_type, index_type>& {
        return m_osqp_problem;
    }
    ...
    auto setHardFences(std::span<const HardFence<scalar_type>> hard_fences) noexcept -> void {
        ...
        for (const auto& hard_fence : hard_fences) {
            ...
            if (hard_fence.actio == ::boyle::bicycle::Actio::PUSHING) {
                if (istart != 0) {
                    ...
                    m_osqp_problem.addConstraintTerm(
        ...
        for (index_type i{1}; i < m_num_samples - 1; ++i) {
            m_osqp_problem.updateConstraintTerm(
    ...
    auto setSoftFences(std::span<const SoftFence1d> soft_fences) noexcept -> void {
        for (const auto& soft_fence : soft_fences) {
            ...
            if (soft_fence.actio == ::boyle::bicycle::Actio::PUSHING) {
                if (istart == iend) {
                    ...
                    m_osqp_problem.addRampCostTerm(
                        {{sIndex(istart - 1), -proration_coeffs[0]},
                         {sIndex(istart), -proration_coeffs[1]},
                         {vIndex(istart - 1), -proration_coeffs[2]},
                         {vIndex(istart), -proration_coeffs[3]},
    ...
    auto setVelocityCost(scalar_type target_velocity, scalar_type velocity_weight) noexcept
        ...
        for (index_type i{0}; i < m_num_samples - 1; ++i) {
            ...
            m_osqp_problem.addQuadCostTerm(sIndex(i), sIndex(i), factor * kFactors[0]);
            m_osqp_problem.addQuadCostTerm(sIndex(i), sIndex(i + 1), -factor * kFactors[1]);
            m_osqp_problem.addQuadCostTerm(sIndex(i), vIndex(i), factor * m_hs[i] * kFactors[2]);
            m_osqp_problem.addQuadCostTerm(
                sIndex(i), vIndex(i + 1), factor * m_hs[i] * kFactors[2]
            );
            m_osqp_problem.addQuadCostTerm(sIndex(i), aIndex(i), factor * m_h2s[i] * kFactors[3]);
            m_osqp_problem.addQuadCostTerm(
                sIndex(i), aIndex(i + 1), -factor * m_h2s[i] * kFactors[3]
            );
            m_osqp_problem.addQuadCostTerm(sIndex(i + 1), sIndex(i + 1), factor * kFactors[0]);
            m_osqp_problem.addQuadCostTerm(
                sIndex(i + 1), vIndex(i), -factor * m_hs[i] * kFactors[2]
            );
            m_osqp_problem.addQuadCostTerm(
            ...
            m_osqp_problem.addQuadCostTerm(vIndex(i), vIndex(i), factor * m_h2s[i] * kFactors[4]);
            m_osqp_problem.addQuadCostTerm(
                vIndex(i), vIndex(i + 1), -factor * m_h2s[i] * kFactors[5]
            );
            m_osqp_problem.addQuadCostTerm(vIndex(i), aIndex(i), factor * m_h3s[i] * kFactors[6]);
            m_osqp_problem.addQuadCostTerm(
                vIndex(i), aIndex(i + 1), factor * m_h3s[i] * kFactors[7]
            );
            m_osqp_problem.addQuadCostTerm(
            ...
            m_osqp_problem.addQuadCostTerm(aIndex(i), aIndex(i), factor * m_h4s[i] * kFactors[8]);
            m_osqp_problem.addQuadCostTerm(
                aIndex(i), aIndex(i + 1), factor * m_h4s[i] * kFactors[8]
            );
            m_osqp_problem.addQuadCostTerm(
                aIndex(i + 1), aIndex(i + 1), factor * m_h4s[i] * kFactors[8]
            );
            m_osqp_problem.addLinCostTerm(sIndex(i), factor * target_velocity * m_hs[i] * 2.0);
            m_osqp_problem.addLinCostTerm(sIndex(i + 1), -factor * target_velocity * m_hs[i] * 2.0);
        }
        return;
    }
    ...
    auto setAccelCost(scalar_type accel_weight) noexcept -> void {
        ...
        for (index_type i{0}; i < m_num_samples - 1; ++i) {
            ...
            m_osqp_problem.addQuadCostTerm(sIndex(i), sIndex(i), factor * kFactors[0]);
            m_osqp_problem.addQuadCostTerm(sIndex(i), sIndex(i + 1), -factor * kFactors[1]);
            m_osqp_problem.addQuadCostTerm(sIndex(i), vIndex(i), factor * m_hs[i] * kFactors[0]);
            m_osqp_problem.addQuadCostTerm(
                sIndex(i), vIndex(i + 1), factor * m_hs[i] * kFactors[0]
            );
            m_osqp_problem.addQuadCostTerm(sIndex(i), aIndex(i), factor * m_h2s[i] * kFactors[2]);
            m_osqp_problem.addQuadCostTerm(
                sIndex(i), aIndex(i + 1), -factor * m_h2s[i] * kFactors[2]
            );
            m_osqp_problem.addQuadCostTerm(sIndex(i + 1), sIndex(i + 1), factor * kFactors[0]);
            m_osqp_problem.addQuadCostTerm(
                sIndex(i + 1), vIndex(i), -factor * m_hs[i] * kFactors[0]
            );
            m_osqp_problem.addQuadCostTerm(
            ...
            m_osqp_problem.addQuadCostTerm(vIndex(i), vIndex(i), factor * m_h2s[i] * kFactors[3]);
            m_osqp_problem.addQuadCostTerm(
                vIndex(i), vIndex(i + 1), factor * m_h2s[i] * kFactors[4]
            );
            m_osqp_problem.addQuadCostTerm(vIndex(i), aIndex(i), factor * m_h3s[i] * kFactors[5]);
            m_osqp_problem.addQuadCostTerm(
            ...
            m_osqp_problem.addQuadCostTerm(
                vIndex(i + 1), aIndex(i), factor * m_h3s[i] * kFactors[6]
            );
            m_osqp_problem.addQuadCostTerm(
                vIndex(i + 1), aIndex(i + 1), -factor * m_h3s[i] * kFactors[5]
            );
            m_osqp_problem.addQuadCostTerm(aIndex(i), aIndex(i), factor * m_h4s[i] * kFactors[7]);
            m_osqp_problem.addQuadCostTerm(
                aIndex(i), aIndex(i + 1), factor * m_h4s[i] * kFactors[8]
            );
            m_osqp_problem.addQuadCostTerm(
    ...
    auto setJerkCost(scalar_type jerk_weight) noexcept -> void {
        for (index_type i{0}; i < m_num_samples - 1; ++i) {
            ...
            m_osqp_problem.addQuadCostTerm(sIndex(i), sIndex(i), factor * 720.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i), sIndex(i + 1), -factor * 1440.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i), vIndex(i), factor * m_hs[i] * 720.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i), vIndex(i + 1), factor * m_hs[i] * 720.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i), aIndex(i), factor * m_h2s[i] * 120.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i), aIndex(i + 1), -factor * m_h2s[i] * 120.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i + 1), sIndex(i + 1), factor * 720.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i + 1), vIndex(i), -factor * m_hs[i] * 720.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i + 1), vIndex(i + 1), -factor * m_hs[i] * 720.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i + 1), aIndex(i), -factor * m_h2s[i] * 120.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i + 1), aIndex(i + 1), factor * m_h2s[i] * 120.0);
            m_osqp_problem.addQuadCostTerm(vIndex(i), vIndex(i), factor * m_h2s[i] * 192.0);
            m_osqp_problem.addQuadCostTerm(vIndex(i), vIndex(i + 1), factor * m_h2s[i] * 336.0);
            m_osqp_problem.addQuadCostTerm(vIndex(i), aIndex(i), factor * m_h3s[i] * 72.0);
            m_osqp_problem.addQuadCostTerm(vIndex(i), aIndex(i + 1), -factor * m_h3s[i] * 48.0);
            m_osqp_problem.addQuadCostTerm(vIndex(i + 1), vIndex(i + 1), factor * m_h2s[i] * 192.0);
            m_osqp_problem.addQuadCostTerm(vIndex(i + 1), aIndex(i), factor * m_h3s[i] * 48.0);
            m_osqp_problem.addQuadCostTerm(vIndex(i + 1), aIndex(i + 1), -factor * m_h3s[i] * 72.0);
            m_osqp_problem.addQuadCostTerm(aIndex(i), aIndex(i), factor * m_h4s[i] * 9.0);
            m_osqp_problem.addQuadCostTerm(aIndex(i), aIndex(i + 1), -factor * m_h4s[i] * 6.0);
            m_osqp_problem.addQuadCostTerm(aIndex(i + 1), aIndex(i + 1), factor * m_h4s[i] * 9.0);
        }
        return;
    }
    ...
    auto setSnapCost(scalar_type snap_weight) noexcept -> void {
        for (index_type i{0}; i < m_num_samples - 1; ++i) {
            ...
            m_osqp_problem.addQuadCostTerm(sIndex(i), sIndex(i), factor * 43200.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i), sIndex(i + 1), -factor * 86400.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i), vIndex(i), factor * m_hs[i] * 43200.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i), vIndex(i + 1), factor * m_hs[i] * 43200.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i), aIndex(i), factor * m_h2s[i] * 7200.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i), aIndex(i + 1), -factor * m_h2s[i] * 7200.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i + 1), sIndex(i + 1), factor * 43200.0);
            m_osqp_problem.addQuadCostTerm(sIndex(i + 1), vIndex(i), -factor * m_hs[i] * 43200.0);
            m_osqp_problem.addQuadCostTerm(
                sIndex(i + 1), vIndex(i + 1), -factor * m_hs[i] * 43200.0
            );
            m_osqp_problem.addQuadCostTerm(sIndex(i + 1), aIndex(i), -factor * m_h2s[i] * 7200.0);
            m_osqp_problem.addQuadCostTerm(
                sIndex(i + 1), aIndex(i + 1), factor * m_h2s[i] * 7200.0
            );
            m_osqp_problem.addQuadCostTerm(vIndex(i), vIndex(i), factor * m_h2s[i] * 10944.0);
            m_osqp_problem.addQuadCostTerm(vIndex(i), vIndex(i + 1), factor * m_h2s[i] * 21312.0);
            m_osqp_problem.addQuadCostTerm(vIndex(i), aIndex(i), factor * m_h3s[i] * 3744.0);
            m_osqp_problem.addQuadCostTerm(vIndex(i), aIndex(i + 1), -factor * m_h3s[i] * 3456.0);
            m_osqp_problem.addQuadCostTerm(
                vIndex(i + 1), vIndex(i + 1), factor * m_h2s[i] * 10944.0
            );
            m_osqp_problem.addQuadCostTerm(vIndex(i + 1), aIndex(i), factor * m_h3s[i] * 3456.0);
            m_osqp_problem.addQuadCostTerm(
                vIndex(i + 1), aIndex(i + 1), -factor * m_h3s[i] * 3744.0
            );
            m_osqp_problem.addQuadCostTerm(aIndex(i), aIndex(i), factor * m_h4s[i] * 336.0);
            m_osqp_problem.addQuadCostTerm(aIndex(i), aIndex(i + 1), -factor * m_h4s[i] * 528.0);
            m_osqp_problem.addQuadCostTerm(aIndex(i + 1), aIndex(i + 1), factor * m_h4s[i] * 336.0);
        }
        return;
    }
    ...
    auto solve() const -> std::pair<Motion1d, ::boyle::cvxopm::Info<scalar_type, index_type>> {
        ...
        const auto [osqp_result, osqp_info] = osqp_solver.solve(m_osqp_problem);
  ...
  private:
    ...
    ::boyle::cvxopm::OsqpProblem<scalar_type, index_type> m_osqp_problem;
