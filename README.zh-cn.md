...
| 文件              | 目标                      | 说明                                                                                                                    |
| ----------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `vec2.hpp`        | `Boyle::math_vec2`        | 固定尺寸 2D 向量 `Vec2<T>`，提供 `euclidean()`、`radian()`、`rotate()`、`crossProj()`、`normalized()`，可选 AVX-512 SIMD |
...
| `qdldl_dcmp.hpp`          | `Boyle::math_qdldl_dcmp`          | 稀疏 LDL^T 分解 `QdldlDcmp<T>`（基于 qdldl），用于求解以上三角 CSC 格式存储的对称准定线性方程组 |
...
| `fixed.hpp`                 | `Boyle::math_fixed`                 | 定点数类型 `Fixed<ValueType, IntermediateType, FractionBits>`，提供完整算术运算符、数学函数（`sqrt`、`sin`、`cos`、`exp`、`log` 等）、`std::numeric_limits` 特化及 `std::formatter` 支持。预定义别名：`Fixed32f8`、`Fixed32f16`、`Fixed32f24` |
...
| 文件                | 目标                          | 说明                                                                  |
| ------------------- | ----------------------------- | --------------------------------------------------------------------- |
| `osqp_problem.hpp`  | `Boyle::cvxopm_osqp_problem`  | 稀疏 QP 问题 `OsqpProblem<T, I>`：最小化 ½xᵀPx + qᵀx，约束 l ≤ Ax ≤ u |
...
| 文件             | 目标                         | 说明                                                                        |
| ---------------- | ---------------------------- | --------------------------------------------------------------------------- |
| `route_line.hpp` | `Boyle::bicycle_route_line` | 参考线 `RouteLine<T>`，基于分段五次曲线，用于自动驾驶                       |
| `path.hpp`       | `Boyle::bicycle_path`       | 2D 路径 `Path<T>`，提供 `tangent()`、`normal()`、`curvature()`、`inverse()` |
| `motion.hpp`     | `Boyle::bicycle_motion`     | 时间-弧长映射 `Motion<T>`，提供 `velocity()`、`accel()`、`jerk()`           |
| `fence.hpp`      | `Boyle::bicycle_fence`      | 时间-空间约束 `HardFence<T>` / `SoftFence<T>`，用于纵向规划                 |
| `border.hpp`     | `Boyle::bicycle_border`     | 空间约束 `HardBorder<T>` / `SoftBorder<T>`，带手性，用于横向规划            |
| `dualism.hpp`    | `Boyle::bicycle_dualism`    | 枚举：`Chirality`（LEFT/RIGHT）、`Actio`（BLOCKING/YIELDING）               |

#### 运动规划器（`bicycle/planners/`）

| 文件                                     | 目标                                                | 说明                                                |
| ----------------------------------------- | ---------------------------------------------------- | --------------------------------------------------- |
| `route_line_cubic_acc_planner.hpp`      | `Boyle::bicycle_route_line_cubic_acc_planner`      | 三次加速度规划器 — 通过 QP 优化生成纵向运动           |
| `route_line_quintic_acc_planner.hpp`    | `Boyle::bicycle_route_line_quintic_acc_planner`    | 五次加速度规划器 — 通过 QP 优化生成更平滑的纵向运动   |
| `route_line_cubic_offset_planner.hpp`   | `Boyle::bicycle_route_line_cubic_offset_planner`   | 三次偏移规划器 — 通过 QP 优化生成横向路径偏移         |
| `route_line_quintic_offset_planner.hpp` | `Boyle::bicycle_route_line_quintic_offset_planner` | 五次偏移规划器 — 通过 QP 优化生成更平滑的横向路径偏移 |
...
OsqpProblem<double, int> osqp_problem(2, 3);   // 2 个变量，3 个约束
...
| `boyle::bicycle`     | 运动规划：路径、参考线、运动、边界、围栏、规划模型    |
